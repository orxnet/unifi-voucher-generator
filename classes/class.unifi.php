...
class Unifi {
	private $user			= UNIFI_USER;
	private $password		= UNIFI_PASSWORD;
	public  $site			= UNIFI_SITE;
	public  $baseurl		= UNIFI_BASEURL;
	public  $version		= UNIFI_VERSION;
	 ...
	 * optional parameter <down>	= download speed limit in kbps
	 * optional parameter <MBytes>	= data transfer limit in MB
	 * optional parameter <codes>	= return only the voucher codes, returns object on false (true by default to be backward compatible)
	 */
	public function create_voucher($minutes, $count = 1, $quota = '0', $note = NULL, $up = NULL, $down = NULL, $MBytes = NULL, $codes = true) {
		if (!$this->is_loggedin) return FALSE;
		$return = array();
		...
		if ($content_decoded->meta->rc == 'ok') {
			if (is_array($content_decoded->data)) {
				...
				foreach ($this->stat_voucher($obj->create_time) as $voucher)  {
					$return[]= ($codes)?$voucher->code:$voucher;
				}
			}
		}
		return $return;
